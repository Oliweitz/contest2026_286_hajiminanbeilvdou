...
################################################################
  READY -- 两种说话方式(对着板子上的麦克风孔):

  [1] 唤醒词(默认方式,不用碰板子):
        先说 "你好openvela",停顿一下,再说指令。例如:
            "你好openvela" -> "现在几点了"
       唤醒后 8 秒内说出指令即可;也可以一口气说完:
            "你好openvela,请每两分钟巡检一次设备"
       ⚠️ 不说唤醒词直接说话会被忽略 -- 这是故意的(否则环境噪声会乱触发)

  [2] PTT 按钮(屏幕下方蓝色圆形按钮,点按切换):
        点一下开始录音 -> 说话 -> 再点一下结束(不是按住说话!)
        PTT 不需要唤醒词。
